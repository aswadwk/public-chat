...
const Bubbles = ({ message, username = "user" }: any) => {
  ...
  useEffect(() => {
    ...
    class Bubble {
      ...
      constructor(text: any, isMoving = true, index = 0) {
        ...
        // Get current date before measuring text dimensions
        const now = new Date();
        ...
        this.width = Math.min(
          Math.max(textMetrics.width, dateMetrics.width) + this.padding * 2,
          canvas.width * 0.7
        );
        const textHeight = this.fontSize + this.padding * 2;
        const dateHeight = this.dateFontSize + this.dateMargin * 2;
        this.height = textHeight + dateHeight;

        // Set targetY for vertical stacking
        this.targetY = 20 + index * (this.height + 20) + this.height / 2;
        // Modify initial y: for moving bubbles start above the canvas
        this.y = isMoving ? -this.height : this.targetY;
      }
    ...
    const animate = () => {
      ...
      // Update target positions for fixed bubbles so they stay in order at the top
      fixedBubbles.forEach((bubble: any, index: any) => {
        bubble.targetY = 20 + index * (bubble.height + 20) + bubble.height / 2;
      });
  ...
  return (
    <div
      ref={containerRef}
      className="relative w-full h-screen overflow-y-auto bg-opacity-50 bg-clip-padding backdrop-filter backdrop-blur-sm"
      style={{
        padding: "20px",
